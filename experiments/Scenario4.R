...
winter_LT <- generate_leadtimes_supplier(N=(20000), C = 6, K = 6, M = 6,alpha = 0.4,
                                         mu_I = 1, mu_D1 = 0.5, mu_D2 = 0.75, mu_D3 = 1,
                                         mu_O = 1)$total_time$sojourn_time
...
#NRMSE
(rmse(output[[3]]$leadtime_demand_retailer[887:7099], (output[[3]]$leadtime_demand_retailer[887:7099]+output[[3]]$errors[887:7099]))/mean(output[[3]]$leadtime_demand_retailer[887:7099]))

#SME
mean(((output[[3]]$errors[887:7099])))/mean(output[[3]]$leadtime_demand_retailer[887:7099])

data <- data.frame(output[[3]]$replenishment_orders_retailer[887:7099], output[[3]]$season[887:7099], 887:7099)
colnames(data) <- c("orders", "season", "id")

ggplot(data, aes(x=season, y=orders)) + geom_jitter(alpha=0.05) + geom_violin(draw_quantiles = c(0.25,0.5,0.75), alpha = 0.1) + ylim(c(0,16000)) + ggtitle("Violin plot of the orders")
...
data %>% group_by(season) %>% summarize(sum(orders > 10000))

data %>% filter(orders < 0)
data %>% filter(orders > 10000)


stat <- matrix(NA, 2,5)
rownames(stat) <- c("rmse", "mean_error")
for (i in 1:length(service_levels)){
  stat[1,i] <-sqrt(mean((na.omit(output_season[[i]]$errors))^2))
  stat[2,i] <- mean((na.omit(output_season[[i]]$errors)))
}
...
for (k in 1:length(service_levels)){
  i=((365)+max(n,m)+4)
  while (sum(((output_season[[k]]$BM_retailer[i]/output_season[[k]]$BM_retailer[(i+1):(length(demand)-max(leadtimes_retailer[observed_leadtimes<=length(demand)]))]) > 1.01 )
             | ((output_season[[k]]$BM_retailer[i]/output_season[[k]]$BM_retailer[(i+1):(length(demand)-max(leadtimes_retailer[observed_leadtimes<=length(demand)]))]) < 0.99)) > 0){
    i = i+1
  }
  convergence_1[k] <- i
}
...
for (k in 1:length(service_levels)){
  j=((365)+max(n,m)+4)
  while (sum(((output_season[[k]]$BM_retailer[j]/output_season[[k]]$BM_retailer[(j+1):(length(demand)-max(leadtimes_retailer[observed_leadtimes<=length(demand)]))]) > 1.05 )
             | ((output_season[[k]]$BM_retailer[j]/output_season[[k]]$BM_retailer[(j+1):(length(demand)-max(leadtimes_retailer[observed_leadtimes<=length(demand)]))]) < 0.95)) > 0){
    j = j+1
  }
  convergence_5[k] <- j
}
...
error_plots <- list()
for (i in 1: length(service_levels)) {
  data <- as.data.frame(cbind(x,output_season[[i]]$BM_retailer, output_season[[i]]$errors, output_season[[i]]$season))
...
for (i in 1: length(service_levels)) {
  mean[i] <- mean(na.omit(output_season[[i]]$BM_retailer))
  sd[i] <- sd(na.omit(output_season[[i]]$BM_retailer))
}
mean 
sd
...
for (i in 1: length(service_levels)) {
  ...
  sd[i] <- sd(na.omit(output_notseason[[i]]$BM_retailer))
...
for (i in 1:length(service_levels)){
  for (j in 1:5){
    mean[j,i] <- mean(na.omit(output_season[[i]]$BM_yearly[(max(m,n)+4+(365*(j))):(max(n,m)+4+(365*(j+1)))]))
    variance[j,i] <- sd(na.omit(output_season[[i]]$BM_yearly[(max(m,n)+4+(365*(j))):(max(n,m)+4+(365*(j+1)))]))
    min[j,i] <- min(na.omit(output_season[[i]]$BM_yearly[(max(m,n)+4+(365*(j))):(max(n,m)+4+(365*(j+1)))]))
    max[j,i] <- max(na.omit(output_season[[i]]$BM_yearly[(max(m,n)+4+(365*(j))):(max(n,m)+4+(365*(j+1)))]))
  }
}
