...
inventory_model_stochastic <- function(m, n, p, demand, leadtimes_retailer){
  ...
  for (i in ((365+1):(length(leadtimes_retailer)))) {
    temp <- leadtimes_retailer[(observed_leadtimes <= (i-1))]
...
for (i in (n+1):(length(demand)-max(leadtimes_retailer[observed_leadtimes<=length(demand)]))) {
  ...
  for (i in ((365)+max(n,m)+1):(length(demand))) {
  ...
  for (i in ((365)+max(n,m)+2):(length(demand))){
  ...
  for (i in (((365)+max(n,m)+3):(length(demand)))){
  ...
  for (i in (((365)+max(n,m)+4):(length(demand)-(round(max(leadtimes_retailer[observed_leadtimes>length(demand)])))))){
    BM_retailer[i] <- ((var(q_t_retailer[((365)+max(n,m)+3):i]))/(var(demand[((365)+max(n,m)+3):i])))
  }
  
  
  BM_yearly <- matrix(NA, length(demand), 1)
  for (j in (1:5)){
    for (i in (((max(n,m)+4+365*j)):(max(n,m)+4+(365*(j+1))))){
      BM_yearly[i] <- ((var(q_t_retailer[((365*j)+max(n,m)+3):i]))/(var(demand[((365*(j+1))+max(n,m)+3):i])))
    }
  }
  
  
  BM_sim <- ((var(q_t_retailer[(max(n,m)+4+365):length(demand)]))/(var(demand)))
